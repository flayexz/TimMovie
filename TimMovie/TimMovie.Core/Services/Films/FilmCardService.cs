...
public class FilmCardService
{
    ...
    public Result<IEnumerable<FilmCardDto>> GetFilmRecommendationsByUserId(Guid userId, int amount)
    {
        ...
        var bestGradedFilm =
            queryExec.IncludeInResult(watched => watched.Film.Producers).GetEntitiesWithPagination(0, 1)
                .FirstOrDefault();
        ...
        if (producer == null)
        {
            return Result.Fail<IEnumerable<FilmCardDto>>("у фильма с лучшим рейтингом нет продюссера");
        }
        
        var filmQuery = _filmRepository.Query
            .Where(new FilmWithProducerSpec(producer.Id));
